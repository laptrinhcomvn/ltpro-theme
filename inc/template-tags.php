 ...
 */
function ltpro_site_logo( $args = array(), $echo = true ) {
	...
	/**
	 * Filters the arguments for `ltpro_site_logo()`.
	 *
	 * @param array  $args     Parsed arguments.
	 * @param array  $defaults Function's default arguments.
	 */
	$args = apply_filters( 'ltpro_site_logo_args', $args, $defaults );
	 ...
	 */
	$html = apply_filters( 'ltpro_site_logo', $html, $args, $classname, $contents );
 ...
 */
function ltpro_site_description( $echo = true ) {
	 ...
	 */
	$html = apply_filters( 'ltpro_site_description', $html, $description, $wrapper );
 ...
 * @return bool
 */
function ltpro_is_comment_by_post_author( $comment = null ) {
 ...
 */
function ltpro_filter_comment_reply_link( $link ) {
...
add_filter( 'comment_reply_link', 'ltpro_filter_comment_reply_link' );
 ...
 */
function ltpro_the_post_meta( $post_id = null, $location = 'single-top' ) {

	echo ltpro_get_post_meta( $post_id, $location ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in ltpro_get_post_meta().
 ...
 */
function ltpro_edit_post_link( $link, $post_id, $text ) {
	if ( is_admin() ) {
		return $link;
	}
	...
	return '<div class="post-meta-wrapper post-meta-edit-link-wrapper"><ul class="post-meta"><li class="post-edit meta-wrapper"><span class="meta-icon">' . ltpro_get_theme_svg( 'edit' ) . '</span><span class="meta-text"><a href="' . esc_url( $edit_url ) . '">' . $text . '</a></span></li></ul><!-- .post-meta --></div><!-- .post-meta-wrapper -->';

}

add_filter( 'edit_post_link', 'ltpro_edit_post_link', 10, 3 );
 ...
 */
function ltpro_get_post_meta( $post_id = null, $location = 'single-top' ) {
	 ...
	 */
	$disallowed_post_types = apply_filters( 'ltpro_disallowed_post_types_for_meta_output', array( 'page' ) );
	...
	if ( 'single-top' === $location ) {
		...
		*/
		$post_meta                 = apply_filters(
			'ltpro_post_meta_location_single_top',
			array(
				'author',
				'post-date',
				'comments',
		...
		*/
		$post_meta                 = apply_filters(
			'ltpro_post_meta_location_single_bottom',
			array(
				'tags',
			)
		);
	...
	if ( $post_meta && ! in_array( 'empty', $post_meta, true ) ) {
		...
		<div class="post-meta-wrapper<?php echo esc_attr( $post_meta_wrapper_classes ); ?>">
			...
			<ul class="post-meta<?php echo esc_attr( $post_meta_classes ); ?>">
				 ...
				 * @param int   $post_ID Post ID.
				 */
				do_action( 'ltpro_start_of_post_meta_list', $post_id );
				...
				if ( in_array( 'post-date', $post_meta, true ) ) {
					...
					<li class="post-date meta-wrapper">
						<span class="meta-icon">
							...
							<?php ltpro_the_theme_svg( 'calendar' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'categories', $post_meta, true ) && has_category() ) {
					...
					<li class="post-categories meta-wrapper">
						<span class="meta-icon">
							...
							<?php ltpro_the_theme_svg( 'folder' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'comments', $post_meta, true ) && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
					...
					<li class="post-comment-link meta-wrapper">
						<span class="meta-icon">
							<?php ltpro_the_theme_svg( 'comment' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'sticky', $post_meta, true ) && is_sticky() ) {
					...
					<li class="post-sticky meta-wrapper">
						<span class="meta-icon">
							<?php ltpro_the_theme_svg( 'bookmark' ); ?>
						</span>
						<span class="meta-text">
				 ...
				 * @param int   $post_ID Post ID.
				 */
				do_action( 'ltpro_end_of_post_meta_list', $post_id );
 ...
 */
function ltpro_filter_wp_list_pages_item_classes( $css_class, $item, $depth, $args, $current_page ) {
...
add_filter( 'page_css_class', 'ltpro_filter_wp_list_pages_item_classes', 10, 5 );
 ...
 */
function ltpro_add_sub_toggles_to_main_menu( $args, $item, $depth ) {
	...
	if ( isset( $args->show_toggles ) && $args->show_toggles ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			...
			$toggle_duration      = ltpro_toggle_duration();

			// Add the sub menu toggle.
			$args->after .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', 'twentytwenty' ) . '</span>' . ltpro_get_theme_svg( 'chevron-down' ) . '</button>';
...
add_filter( 'nav_menu_item_args', 'ltpro_add_sub_toggles_to_main_menu', 10, 3 );
 ...
 */
function ltpro_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'social' === $args->theme_location ) {
		$svg = LTPro_SVG_Icons::get_social_link_svg( $item->url );
		if ( empty( $svg ) ) {
			$svg = ltpro_get_theme_svg( 'link' );
		}
...
add_filter( 'walker_nav_menu_start_el', 'ltpro_nav_menu_social_icons', 10, 4 );
...
add_action( 'wp_head', 'ltpro_no_js_class' );
 ...
 */
function ltpro_body_classes( $classes ) {
...
add_filter( 'body_class', 'ltpro_body_classes' );
 ...
 */
function ltpro_get_the_archive_title( $title ) {

	$regex = apply_filters(
		'ltpro_get_the_archive_title_regex',
		array(
			'pattern'     => '/(\A[^\:]+\:)/',
...
add_filter( 'get_the_archive_title', 'ltpro_get_the_archive_title' );
 ...
 */
function ltpro_toggle_duration() {
	/**
	 ...
	 */
	$duration = apply_filters( 'ltpro_toggle_duration', 250 );
 ...
 */
function ltpro_unique_id( $prefix = '' ) {
	static $id_counter = 0;
